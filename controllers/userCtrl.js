...
const applyMechanicController = async (req, res) => {
  try {
    const newMechanic = await mechanicModel({ ...req.body, status: "pending" });
    await newMechanic.save();
    const adminUser = await userModel.findOne({ isAdmin: true });
    const notifcation = adminUser.notifcation;
    notifcation.push({
      type: "apply-mechanic-request",
    ...
    res.status(500).send({
      success: false,
      error,
      message: "Error WHile Applying For Mechanic",
    });
  }
};
...
const getAllMechanicsController = async (req, res) => {
  try {
    const mechanics = await mechanicModel.find({ status: "approved" });
    res.status(200).send({
      success: true,
      message: "Mechanic Lists Fetched Successfully",
      data: mechanics,
    });
  } catch (error) {
    console.log(error);
    res.status(500).send({
      success: false,
      error,
      message: "Errro WHile Fetching Mechanic",
    });
  }
};
...
const bookeAppointmnetController = async (req, res) => {
  try {
    ...
    const user = await userModel.findOne({ _id: req.body.mechanicInfo.userId });
